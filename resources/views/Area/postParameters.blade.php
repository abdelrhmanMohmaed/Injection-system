                        ...
                        <div class="media-body">
                            ...
                            <div class="form-group text-center">
                                <input type="hidden" class="form-control" value="{{$machine->id}}"
                                       name="machine_id">
                                <input type="hidden" class="form-control" value="{{$post->part_id}}"
                                       name="part_id">
                                <button type="submit" id="changeParameter"  style="margin-top: 23px;" class="btn btn-info">Save</button>
                            </div>
                                @endif
                        </div>
                    </div>
...
<script>
    ...
    $('#changeParameter').click(function (e) {
        e.preventDefault();
        var formData=$('#ChangeParameterForm').serializeArray();
        $.ajax({
            type: 'post',
            url: ' {{url('change/post/parameter/'.$post->id)}}',
            data:formData,
    ...
    $(document).ready(function(){
        $.ajax({
            type: 'get',
            url: '{{url('Parameter/byBn/')}}/'+'{{$post->bn_id}}',
            success: function (res) {
                if (res) {
